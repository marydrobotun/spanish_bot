...
from base import Language, UserInfo, UserStats
...
dp = Dispatcher()
dp.message.middleware(UserLanguageMiddleware(redis, user_repo))
dp.callback_query.middleware(UserLanguageMiddleware(redis, user_repo))
...
async def text_handler(message: Message, lang: str) -> None:
    ...
    if message.from_user.id in current_trainings.keys():
        ...
        if message.text.strip() == current_trainings[message.from_user.id][1]:
            ...
            await message.answer(
                MessageGetter.get_right_answer_message(lang=Language(lang)),
                reply_markup=builder.as_markup(),
            )
        else:
            await message.answer(
